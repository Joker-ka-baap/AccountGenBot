...
Bot Made With ❤️ By  @Free_Netflix_AccountzZ **
""" 
    await event.reply(user_text) 
    
@warnerstarkbot.on(events.NewMessage(pattern="^/(help|cmds)$"))
...
/netflix - To Generate Netflix Accounts
...
@warnerstarkbot.on(events.NewMessage(pattern="^/(netflix|Netflix)$"))
async def hmm(event):
    if Config.JTU_ENABLE:
    	starky = await check_if_subbed(Config.CHANNEL_USERNAME, event, warnerstarkbot)
    	if starky is False:
    ...
    if warner.is_user_in_db(int(event.sender_id)):
        ...
        if warner.is_user_in_db(int(event.sender_id)) >= Config.GEN_LIMIT_PERDAY:
            await hmmw.edit(f"**Your Daily Limit is exhausted, Kindly Contact the admins to increase ur limit\n\nBy The Way Daily Limit is 1 accounts per day**", buttons=[[custom.Button.url("Join Channel", Config.CHANNEL_URL)]])
            return
    ...
    await hmmw.edit(
        f"**Netflix Account Generated \n\nCombo: {email}:{password} \nEmail: {email} \nPassword: {password} \nPlan: Premium \nExpiry: \nTrial: False  \nYou Can Check Your Limit or Info By /about \n\nUser-ID: {event.sender_id} \nGenerated By @{username}**", buttons=[[Button.url("Join Channel!", "t.me/Free_Netflix_AccountzZ/")]],
        parse_mode="Markdown")
